...
public class Prim_PQ {
	...
	static class Graph {
		...
		void MST()
		{
			...
			PriorityQueue<Pair<Pair<Integer, Integer>,Integer> >
			...
			while (addedEdges.size() < vertices - 1) {
				...
				if (!addedVertices.contains(pair.a.b)) {addedVertices.add(pair.a.b);addedEdges.add(pair);

					// Adding all the connected vertices with vertex B
					iterator = edges[pair.a.b].listIterator();
					while (iterator.hasNext()) {
						Pair<Integer, Integer> pair1= iterator.next();priorityQueue.add(new Pair<>(new Pair<>(pair.a.b, pair1.a),pair1.b));
					}
				}
			}
